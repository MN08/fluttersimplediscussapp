...
import 'package:discuss_app/sources/user_source.dart';
...
class ProfileFragment extends StatelessWidget {
  ...
  updateImage(BuildContext context) {
    ImagePicker().pickImage(source: ImageSource.gallery).then((image) {
      if (image != null) {
        ...
        DInfo.dialogConfirmation(context, 'Update Image', 'update Image ?')
            .then((yes) async {
          if (yes ?? false) {
  ...
  Widget build(BuildContext context) {
    ...
    if (cUser.data == null) return DView.textTitle('text');
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: DView.textTitle('Profile', size: 24),
        ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: GestureDetector(
                            ...
                            child: statItem('Follower', _.stat['follower'])),
                      ),
                      Container(
                        color: Colors.blueGrey,
                        height: 30,
                      ...
                      Expanded(
                        child: GestureDetector(
                          onTap: () {
                            context.push(AppRoute.following, extra: cUser.data);
                          },
                          child: statItem('Following', _.stat['following']),
                        ),
                      ),
                    ],
                  ),
